export default {
  ...
  getStoryFromNotes(notes) {
    ...
    for (let i=0; i<splitNotes.length; i++) {
      if (splitNotes[i] != "" && !this.containsLowercase(splitNotes[i])) chapterIndices.push(i);
    }

    for (let i=0; i<chapterIndices.length-1; i++) {
			let chapter = {};
			chapter.title = splitNotes[chapterIndices[i]];
			chapter.events = splitNotes.slice(chapterIndices[i]+1, chapterIndices[i+1]);
